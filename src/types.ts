...
export function getGridCapacity(tileCount: number): number {
  for (const capacity of GRID_CAPACITIES) {
    if (tileCount <= capacity) return capacity;
  }
  return 25;
}

export function getGridConfig(capacity: number): { cols: number; rows: number } {
  switch (capacity) {
